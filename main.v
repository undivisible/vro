...
fn print_vro_help() {
	...
	println('')
	println('Usage: vro [options] [file ...]')
	println('')
	println('Options:')
	...
	println('')
	println('Keys:')
	println('  Ctrl-S  save       Ctrl-Q  quit (press three times if dirty)')
	println('  Ctrl-F  find       Ctrl-E  command bar (:w, :q, :open, ...)')
	println('  Ctrl-Z  undo       Ctrl-Y  redo')
	println('  Ctrl-C  copy       Ctrl-X  cut')
	println('  Ctrl-V  paste      Ctrl-N  next word completion')
	println('  Ctrl-U  delete to line start')
	println('  Ctrl-W / Option-Delete  delete previous word')
	println('  Ctrl-Delete             delete next word')
	println('  Shift-arrows select when the terminal sends them')
	println('')
	println('Editing: Tab indent; .html/.htm only: Tab expands tags at EOL (emmet-lite).')
	println('Line numbers in the left gutter. Mouse: drag select, double-click word, triple-click sentence.')
	println('')
	println('Env:')
	println('  VRO_NO_MOUSE=1              disable mouse')
	println('  VRO_NO_SYSTEM_CLIPBOARD=1   disable OS clipboard')
	println('  NO_COLOR / VRO_NO_HL=1      disable highlighting')
	println('  VRO_FORCE_COLOR=1             force color')
}
